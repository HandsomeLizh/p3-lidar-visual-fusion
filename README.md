...
- IMU：可选；104 实机使用 Ouster 内置 IMU 做预测与扫描补偿，仿真配置继续支持无 IMU。接入条件见 [可选 IMU](docs/optional_imu.md)。
...
**现场已发现未解决的长时间配准失败：** 2026-09-16 16:01 启动的运行中，LiDAR 连续失败超过 500 帧；视觉虽然仍能跟踪并提供运动增量，融合位姿因不确定性超限被停发。现有视觉接续不能保证长时间失配后的恢复，详见 [现场故障记录](docs/live_failure_20260916.md)。这项问题独立于本轮地图界面更新，尚未修复。
