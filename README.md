...
- LiDAR：VoxelMap 平面及方向协方差、配准质量检查、收敛检查；逐帧定位，移动 0.1 米或转动 1 度后才向内部配准地图加入新视角，避免停车期间反复累积同一批平面证据。
...
- 车辆反馈：独立轻量进程读取现有 6668 接收器的 ROS 速度消息，角速度按度/秒转换。原始速度坐标尚未标定，默认不参与位姿积分；它不是完整 IMU。详见 [车辆速度反馈与 P4 核查](docs/velocity_feedback.md)。
