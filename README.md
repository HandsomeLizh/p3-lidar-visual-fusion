...
**104 车机真实传感器部署**：见 [实机启动与接口](docs/HARDWARE104_CN.md)。使用真实 Ouster＋IMU＋双目，小图在驱动侧生成；默认建图 ROS 域 59。
...
- 恢复：无 IMU 分支可用合格视觉初值，并在持续失配时尝试候选局部子图，经多帧确认后接回。候选失败不打断合格视觉，也不清空累计地图。见 [视觉接续与子图恢复](docs/visual_recovery_20260916.md)。
- IMU：可选；104 实机使用 Ouster 内置 IMU 做预测与扫描补偿，IMU 正常时不用视觉覆盖雷达初值。仿真配置继续支持无 IMU。接入条件见 [可选 IMU](docs/optional_imu.md)。
...
视觉可在质量合格时接续定位。104 实机配置另启用双目稀疏补图：复用可靠三维匹配、按不确定性过滤并经两帧确认后填入未观测栅格，后续雷达观测优先。未实现稠密双目重建；没有可信地图位姿时不入图。见 [实机说明](docs/HARDWARE104_CN.md)。
...
[config/fusion.rviz](config/fusion.rviz) 是独立 RViz 的备用配置。组合窗口源码位于 `visual/rviz_window`，可用 `scripts/build_visual_window.sh` 构建，配置文件也保存在本仓库。
