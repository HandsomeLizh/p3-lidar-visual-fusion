...
**本次交付状态（2026-09-16）**：代码、依赖、驱动、标定配置和窗口已部署、编译；最新雷达＋IMU 静态连通性通过。视觉在较早一轮运行过，最新版完整联调受当前内存压力阻塞。按用户要求保留原有 GLIM。详细结果见 [验证记录](HARDWARE104_VERIFICATION.md)。

目前原有 GLIM 使用约 45～47 GiB 内存，系统已有大量交换区占用。启动脚本要求至少 **4 GiB 可用内存**，不足时会明确退出，不会替用户停止其他程序。此值是根据本模块连同窗口约 2.7 GiB 的实测峰值留出的启动余量，长期大地图仍需监控内存。
...
**双目三维点的用途**：视觉正常运行时通过左右匹配恢复稀疏三维点，用于运动估计和几何一致性检查，尚未直接写入高程或点云地图。当前实机地图几何来自 LiDAR，视觉通过修正位姿帮助拼图。LiDAR 仍有可用点云但配准失败时可用合格视觉定位；原始点云完全断流时，当前没有纯双目新增地图功能。
