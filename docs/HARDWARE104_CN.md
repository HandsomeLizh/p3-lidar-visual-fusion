...
**最新地图更新优化（2026-09-16）**：减少重复体素写盘，批量统计概览栅格；同输入地图计算中位数 0.518→0.295 秒，地图数值一致。相邻两轮实机测试中，局部图数据年龄 2.79→2.22 秒、全局 2.93→2.26 秒，局部约 0.92 Hz。新版 108/108 帧雷达＋IMU 有效，本轮真实双目补图也通过链路检查；运动精度和长时间运行仍需验证，详见 [本轮地图延迟优化](HARDWARE104_PIPELINE_LATENCY_20260916.md)。
