...
局部／全局高程图和地图保存包含这些补图格；`/T3/mapping/lidar_map` 保持纯雷达，组合窗口叠加 `/T3/mapping/stereo_map`，其预览上限 2 万点，待确认缓存上限 4096 格。`/fusion/map_status` 的 `stereo_reason` 解释等待确认、位姿不合格或已有雷达观测等状态。近地表筛选会排除头顶点；双目优先复用近期近邻雷达观测的地面，参考失效时需要用足够的双目观测重新建立支持面。

有可信全局位姿、可观测的支持面且满足入图不确定性阈值时，可以在 LiDAR 点云中断后继续由双目新增稀疏地图；无法建立视觉参考、支持面或位姿误差增大时会暂停。它不是稠密双目重建，未观测空隙仍为未知，不能据此保证纯视觉长期导航。稀疏点不提供完整垂直表面包络；高差可由下游依据相邻已确认栅格判断。
