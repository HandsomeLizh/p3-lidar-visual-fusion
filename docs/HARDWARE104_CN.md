...
**最新要求：双目建图、融合定位。** `mapping_source: stereo`；定位仍使用现有视觉/LiDAR/IMU 融合，地图三维点和高程全部来自双目观测。切换此模式要停止旧运行、创建新地图并重启显示，不能续写旧雷达地图。实现及验证见 [双目地图说明](HARDWARE104_STEREO_ONLY_20260917.md)。
...
**几何只用双目**：`mapping_source: stereo`、`stereo_mapping.enabled: true`。复用现有双目与时序匹配验证过的三维点；每秒最多 2 帧、每帧最多 512 点。用正式融合位姿变换后累积到磁盘体素地图，再发布有数量上限的全局点云预览。高程使用双目观测拟合的近地表、位姿协方差与连续帧确认。建图端不订阅 LiDAR/ToF 点云、不发布 `/T3/mapping/lidar_map`。定位仍按原融合策略运行；视觉质量不足时暂停增加地图，保留已有地图。
...
此前雷达建图模式的过滤验证：146 帧旧记录中过滤 2,973 个范围内点，范围外点保持保留；两分钟静止实机验证剔除 5,511 点，最终过滤盒内点云为 0。这些属于旧 `range` 建图模式，不代表新版双目地图性能。新版结果见双目地图说明。
