...
更新：2026-09-17。默认不带参数的入口采用 **VoxelMap＋XFeat/LighterGlue 定位，只有双目点云建图**；当前手动试车应选择本页的 **视野内 LiDAR 建图**配置，也可直接按 [216 桌面使用说明](DESKTOP_QUICKSTART_CN.md) 启动。仿真配置当前不融合 IMU。日常保留两个启动入口：**采集＋人工控制**、**建图＋RViz**；P4 由使用者另行管理。
...
自动启动缺少的 capture、车辆桥接和控制 GUI，复用配置一致的已有节点。本次连接 UE `192.168.10.23`：采集端口 **6665**，控制与反馈端口 **6668**，ROS 域 **10**。地址变化时修改 `--host`。capture 使用本套项目安装的并行版，2026-09-17 21:14 已切换；无需为了启动建图再次重启 capture。
...
- **采集频率**：请求上限 2 Hz；9 月 17 日晚并行版短时观察约 **1.06 Hz**，不是稳定 2 Hz，实际随 UE 和传输耗时变化。发布频率调高不能生成新的传感器观测。
...
| 控制窗口没打开 | `./start_simulation_sources.sh --host 192.168.10.23 --check` 检查；确认查看的是远程桌面，缺少节点时运行启动脚本 |
...
2026-09-17 晚间恢复更新：LiDAR 单独保留最后可信帧的视觉参考，并检查后续有效视觉增量的连续性、运动边界与累计不确定性，解除旧的 6 秒恢复锁死和普通缓存淘汰问题。图像订阅深度改为 2，处理队列仍只保留最新一对；记录回调、处理、输出时间，便于区分延迟环节。已通过 8 组隔离 ROS 回归并安装，未进行本版车辆运动精度验收。详见 [长间隔恢复修复](visual_gap_recovery_20260917.md)。IMU 按用户要求暂停，不接入融合。
