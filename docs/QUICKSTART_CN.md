...
- **点云参数**：VoxelMap 根体素 **2 米**，输入降采样 **0.2 米**，迭代上限 **30 次**，收敛后提前结束；左侧显示采样 **0.25 米、最多 10 万点**。这些参数作用不同。
- **互相接续**：仿真配置在两路合格时优先使用连续视觉位姿，LiDAR 继续建图与交叉校验；视觉失效或过时，切回合格 LiDAR／EKF。持续失配会尝试候选子图；候选失败不打断合格视觉。两路都失效时停止可信定位输出。见 [视觉优先逻辑](visual_preference_20260916.md)。
...
2026-09-16 修复了短暂失败立即重置视觉的问题：最多保留两个可信参考帧，在 30 秒内尝试几何恢复，成功后延续原轨迹。参考超时或确实无重叠时仍需重新对齐。已通过图像遮黑回放与 ROS 回归；在线行驶仍待验收，详见 [修复记录](visual_recovery_gap_20260916.md)。已启动会话不会自动加载新代码和参数。
