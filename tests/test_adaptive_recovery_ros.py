...
output=ROOT/'results/adaptive_guard_recovery_regression_20260916'
...
profile['independent_visual_min_score']=.45
...
def stage(start,n,black,lidar_good,epoch,visual_offset=0.,visual_score=1.):
    for i in range(start,start+n):
        ...
        drain(.015)
        if not black:raw_visual.publish(odom(stamp,visual_offset+i*.02,(np.array([.01]*3+[.005]*3)/visual_score**2).tolist(),epoch))
        drain(.06)
drain(.5)
try:
    ...
    timing_before=guard.visual_timing_rejected
    constraints_before=guard.visual_constraints
    stage(114,8,False,True,'learned_epoch_10',-900.,visual_score=.60)
    assert guard.visual_constraints==constraints_before+8
    assert guard.visual_timing_rejected==timing_before
    result=dict(passed=True,moderate_confidence_keeps_tracking=True,reference_wait_larger_than_age_is_bounded=True,visual_waits_for_same_stamp_lidar=True,same_epoch_alignment_preserved=True,
        reset_aligned_to_current_lidar=True,unobserved_reset_rejected=True,
        resumed_position_m=poses[-1],expected_position_m=121*.02,
