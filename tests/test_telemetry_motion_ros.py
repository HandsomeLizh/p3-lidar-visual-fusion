...
class TelemetryRosTests(unittest.TestCase):
    ...
    def test_actor_body_velocity_after_legacy_receiver(self):
        self.run_case(False,actor=True)

    def run_case(self, world, actor=False):
        ...
        if actor:out=ROOT/'results/velocity_input_20260916/actor_ros_transport'
        ...
        with tempfile.TemporaryDirectory(dir=ROOT/'build',prefix='telemetry_ros_') as directory:
            ...
            cfg['telemetry_motion'].update(source_domain=62,enabled=True,fuse_velocity=True,
                ...
                input_encoding='legacy_driver_from_actor_body_wire' if actor else 'legacy_driver_from_flu_wire',
                velocity_frame='world' if world else 'body',wire_frame='actor_body' if actor else ('world' if world else 'body_flu'),
            ...
            def message(v=0.,w=0.,age=0.,frame='base_link'):
                ...
                if actor:
                    # Forward is raw -X, left yaw is raw +Y under the Actor
                    # axial-vector convention. These two fields survive the
                    # C++ decoder unchanged; body mode must ignore NaN pose.
                    msg.twist.twist.linear.x=-v
                    msg.twist.twist.angular.z=0.
                    msg.twist.twist.angular.y=w
                if world:
