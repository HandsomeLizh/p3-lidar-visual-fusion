...
class StereoNormalizer:
 ...
 def convert(self,msg,side):
  ...
  encoding=self.cfg.get('normalized_image_encoding','mono8')
  if encoding=='bgr8' and msg.encoding not in ('mono8','8UC1','mono16','16UC1'):
   mono=self.bridge.imgmsg_to_cv2(msg,desired_encoding='bgr8')
  ...
  if encoding=='bgr8' and mono.ndim==2:mono=cv2.cvtColor(mono,cv2.COLOR_GRAY2BGR)
  out=self.bridge.cv2_to_imgmsg(mono,encoding=encoding);out.header=copy.deepcopy(msg.header)
