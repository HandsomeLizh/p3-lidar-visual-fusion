...
OUT = ROOT / 'results/xfeat_only_long_80m_20260915'
...
def main():
    ...
    for path in (ROOT/"results").glob("*/process_state.json"):
        ...
        if int(previous.get("domain",-1))==61 and any(alive(v) for v in previous.get("processes",{}).values()):
            raise RuntimeError("An owned comparison is already active in this domain: "+str(path.parent))
    OUT.mkdir(exist_ok=False)
    state = dict(output=str(OUT), domain=61, processes={})
    ...
    env.update(ROS_DOMAIN_ID='61', ROS_LOCALHOST_ONLY='1',
    ...
    try:
        (OUT/'benchmark_environment.json').write_text(json.dumps(dict(
            source_program=str(ROOT), ros_domain_id=61, localhost_only=True,
            algorithm='XFeat + LighterGlue stereo odometry only; no guard/EKF or LiDAR/IMU',
            ...
            note='Only stereo images and /clock are published. Shared Orin load limits timing comparisons.'), indent=2))
        start('mapper', ['ros2','launch',str(ROOT/'scripts/visual_only.launch.py'),
                         'profile:='+str(OUT/'profile.yaml'),'output_dir:='+str(OUT)])
        ...
        player = start('player', ['/usr/bin/python3', str(ROOT/'scripts/replay_bag.py'),
            ...
            '--frames','174','--images-only', '--report',str(OUT/'replay.json'),
